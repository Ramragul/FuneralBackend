...
app.patch('/api/tailoring/orders/:orderId/update', (req, res) => {

  const { orderStatus, orderAssignment, updatedBy, appointmentDate } = req.body;
  const orderId = req.params.orderId;
  ...
  const fetchQuery = `
    SELECT order_status 
    FROM CC_Tailoring_Orders 
    WHERE order_id = ?
  `;

  con.query(fetchQuery, [orderId], (err, rows) => {
    ...
    con.query(
      ...
      (updateErr) => {
        ...
        // 🔥 Step 3: Update appointment date if provided
        if (appointmentDate) {

          const updateAppointmentQuery = `
            UPDATE CC_Tailoring_Order_Details tod
            INNER JOIN CC_Tailoring_Orders o
            ON o.tailoring_details_id = tod.tailoring_id
            SET tod.appointment_date = ?
            WHERE o.order_id = ?
          `;

          con.query(
            updateAppointmentQuery,
            [appointmentDate, orderId],
            (apptErr) => {

              if (apptErr) {
                console.error("Appointment Update Error:", apptErr);
                return res.status(500).json({
                  error: "Order updated but appointment failed"
                });
              }

              return res.status(200).json({
                message: "Order & Appointment Updated Successfully"
              });
            }
          );
